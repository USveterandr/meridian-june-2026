...
// ───────────────────────────────────────────────────────────────────────────
// remax.com.do listing card parser + normalizer.
//
// ⚠️ SCAFFOLDING — NOT YET VERIFIED AGAINST A REAL RESPONSE ⚠️
// This environment could not resolve remax.com.do to inspect real markup,
// so the class names below (`.listing-card`, `.property-title`, etc.) are
// generic guesses based on common IDX/real-estate-franchise templates —
// not confirmed. Before relying on this:
//   1. Run one ScrapingBee request against a remax.com.do search/listing
//      page with render_js=true and inspect the raw HTML returned.
...
export const REMAX_SEARCH_URL = 'https://www.remax.com.do/listings/sale';

export interface RemaxCard {
  href: string | null;
  title: string;
  price: string;
  location: string;
...
export function parseRemaxCards(html: string): RemaxCard[] {
  return extractBlocks(html, 'div', 'listing-card').map((block) => ({
...
export function normalizeRemaxCard(card: RemaxCard): ScrapedProperty | null {
...
export async function fetchRemaxListings(apiKey: string): Promise<ScrapedProperty[]> {
  const html = await fetchRenderedHtml(apiKey, REMAX_SEARCH_URL, { renderJs: true, waitFor: 2000 });
  return parseRemaxCards(html)
    .map(normalizeRemaxCard)
